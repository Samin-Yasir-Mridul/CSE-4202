...
for(int i=0; i<=row; i++)
{
    if(para[i][1]>para[location][1])
    {
        location= i+1;
    }

}
    printf("\nMaximum Airspeed= %d Knots at time= %d\n", para[location][1], para[location][0]);

//Maximum altitude
int i1, location1=1;
for(int i1=0; i1<=row; i1++)
{
    if(para[i1][2]>para[location1][2])
    {
        location1= i1+1;
    }

}
    printf("\nMaximum Altitude= %d feet at time= %d\n", para[location1][2], para[location1][0]);

//Maximum temp
int i2, location2=1;
for(int i2=0; i2<=row; i2++)
{
    if(para[i2][3]>para[location2][3])
    {
        location2= i2+1;
    }

}
    printf("\nMaximum Temperature= %d Celcius at time= %d\n", para[location2][3], para[location2][0]);
